...
export class BorderColorFiller extends SimpleProcessNode {
    ...
    get ownBridge() {
        if (!this.bridge) {
            this.bridge = new ModelBridge({ "preserve": true }, {
                "properties": [
                    {
                        "name": "preserve",
                        "editor": "boolean",
    ...
    async processImage(buffer) {
        ...
        if (!preserve)
            buffer = this.logicalCrop(buffer);
    ...
    logicalCrop(buffer) {
        ...
        const result = new ByteImageBuffer(new DataView(bytes.buffer, bytes.byteOffset + byteOffset, buffer.pitch * newHeight), newWidth, newHeight, buffer.pitch);
